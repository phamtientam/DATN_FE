<template>
  <div class="body">
    <div id="list_button">
      <button @click="showChart_1">Số Lượng Nhân Viên Theo Độ Tuổi</button>
      <button @click="showChart_2">Số Lượng Nhân Viên Theo Phòng Ban</button>
      <button @click="showChart_3">Nhân Viên Có Gia Đình Và Chưa Có Gia Đình</button>
    </div>
    <div id="type_chart">
      <div>
  ...
  methods: {
    handleChartUpdate() {
      ...
      if (this.charShow === "showChart_1") {
        ...
        this.showChart_2();
      } else if (this.charShow === "showChart_3") {
        this.showChart_3();
      }
    },
    getRandomColor() {
    ...
    async showChart_1() {
      try {
        ...
        const response = await getData("employee/avg_age/", {});
    ...
    async showChart_2() {
      try {
        ...
        const response = await getData("employee/employee_by_department/", {});
        this.process_data_chart(response);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    ...
    async showChart_3() {
      try {
        this.charShow = "showChart_3";
        const response = await getData("employee/employee_by_family_and_single/", {});
        this.process_data_chart(response);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
